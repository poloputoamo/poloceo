...
export const App = () => (
  <Canvas flat camera={{ fov: 75, position: [0, 0, 20] }} eventSource={document.getElementById('root')} eventPrefix="client">
    ...
    <Frame id="01" name={`POLO`} author="ya haré" bg="#e4cdac" position={[-1.15, 0, 0]} rotation={[0, 0.5, 0]}>
      <Gltf src="pickles_3d_version_of_hyuna_lees_illustration-transformed.glb" scale={8} position={[0, -0.7, -2]} />
    </Frame>
    <Frame id="02" name="." author="algo decente" fontsiz={1}>
      <Gltf src="fiesta_tea-transformed.glb" position={[0, -2, -3]} />
    </Frame>
    <Frame id="03" name="CEO" author="algun día" bg="#d1d1ca" position={[1.15, 0, 0]} rotation={[0, -0.5, 0]}>
...
function Frame({ id, name, author, bg, width = 1, height = 1.61803398875, children, fontsiz, ...props }) {
  const portal = useRef()
  ...
  const displayNameFontSize = fontsiz !== undefined ? fontsiz : 0.3
  return (
    <group {...props}>
      <Text font={suspend(medium).default} fontSize={displayNameFontSize} anchorY="top" anchorX="left" lineHeight={0.8} position={[-0.375, 0.715, 0.01]} material-toneMapped={false}>
        {name}
      </Text>
